...
fn collect_function_mutations(
    ...
    // Skip all decorated functions — decorators can transform the function object
    // in ways incompatible with the trampoline (e.g., @property produces a descriptor,
    // @contextmanager wraps the generator, @lru_cache wraps with a cache layer).
    // This matches mutmut's behavior. A future improvement will strip decorators from
    // renamed originals, allowing decorated functions to be mutated safely.
